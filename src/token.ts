 ...
 *   - Signature verification is performed by the backend (X-API-Key).
 */
...
export type ParsedMemberToken = {
  /** Trimmed raw JWT — pass straight to the X-API-Key header. */
  raw: string;
...
function tryDecodeBase64UrlJson(segment: string): Record<string, unknown> | undefined {
...
function readNumericDate(rec: Record<string, unknown>, key: string): number | undefined {
  const v = rec[key];
  const n =
    typeof v === 'number'
      ? v
...
function readAudience(rec: Record<string, unknown>): readonly string[] | undefined {
  const aud = rec['aud'];
